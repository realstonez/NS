...
# -*- coding: utf-8 -*-
"""
Created on Sat May 11 11:47:41 2024

@author: realmac
"""

# AR_simulation.py
# Focusing more on an armament race of hawks and pigeions
...
def run_simulation(variables):
    ...
    [Pigeon_maxSpeed, Hawk_maxAggressiveness, Hawk_birthRate, gridSize,num_generations, density_limit] = variables
    ...
    for _ in range(num_generations):
        ...
        for hawk in hawks:
            '''
            if 0 <= hawk.location[0] < gridSize and 0 <= hawk.location[1] < gridSize:
                hawk_density[hawk.location[0]][hawk.location[1]] += 1
            else:
                print(f"Invalid hawk location: {hawk.location}")
                '''
            hawk_density[hawk.location[0]][hawk.location[1]] += 1
        for j, hawk in enumerate(hawks):
            if hawk_density[hawk.location[0]][hawk.location[1]] > 1:
                ...
                hawks_fight.add(j)
        hawks = [hawk for hawk in hawks if hawk.Energy > 0] # Remove exhausted hawks after fighting

        # Hunting --> hawk didn't fight can hunt
        pigeon_hunted = set()
        hawk_hunting = set()
        for j, hawk in enumerate(hawks):
            if j not in hawks_fight:
                ...
                for i, pigeon in enumerate(pigeons):
                    ...
                    if distance <= hawk.huntingBoundary and random.random() >= np.log(pigeon.speed/hawk.aggressiveness):
        ...
        hawks += [Hawk(hawk.location, hawk.aggressiveness, gridSize) for hawk in hawks if random.random() < Hawk_birthRate]
        pigeons += [Pigeon(pigeon.location, pigeon.speed, gridSize) for pigeon in pigeons if random.random() < pigeon.breedingRate and pigeon_density[pigeon.location[0]][pigeon.location[1]] < density_limit]
